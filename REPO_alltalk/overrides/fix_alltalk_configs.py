        ...
        for key, value in d.items():
            ...
            if "voice" in key.lower():
                if value in placeholders:
                    ...
                    voice_path = Path(__file__).parent / "voices" / value
...
def synchronize_voice_settings(default_voice):
    ...
    configs_to_sync = {
        "Main Config": base_dir / "confignew.json",
        "Remote Config": base_dir / "system" / "TGWUI_Extension" / "tgwui_remote_config.json",
        "Model Settings": base_dir / "system" / "tts_engines" / "xtts" / "model_settings.json"
    }
...
def main():
    ...
    base_dir = Path(__file__).parent
    if not (base_dir / "confignew.json").exists() and not (base_dir / "system").exists():
    ...
    configs = [
        (base_dir / "confignew.json", "Main Configuration"),
        (base_dir / "system" / "TGWUI_Extension" / "tgwui_remote_config.json", "Remote Configuration"),
        (base_dir / "system" / "tts_engines" / "xtts" / "model_settings.json", "Model Settings"),
    ]
    ...
    print("2. Run test to verify fixes")
