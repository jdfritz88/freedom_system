...
# Complete version with all features and diagnostic logging
# ==========================================
...
def log(msg):
    ...
    try:
        diagnostic_log_path = os.path.join(EXTENSION_DIR, 'diagnostic_log.txt')
        with open(diagnostic_log_path, 'a', encoding='utf-8') as f:
            f.write(f"{formatted_msg}\n")
    except Exception as e:
        print(f"[Boredom Monitor] ERROR writing to diagnostic log: {e}")

def diagnostic_startup_log():
    """Comprehensive diagnostic logging at startup"""
    try:
        diagnostic_log_path = os.path.join(EXTENSION_DIR, 'diagnostic_log.txt')
        
        # Clear previous diagnostic log
        with open(diagnostic_log_path, 'w', encoding='utf-8') as f:
            f.write(f"=== BOREDOM MONITOR DIAGNOSTIC LOG ===\n")
            f.write(f"Started: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Extension Directory: {EXTENSION_DIR}\n\n")
        
        log("DIAGNOSTIC: Extension file loaded - Python import successful")
        log(f"DIAGNOSTIC: Extension directory: {EXTENSION_DIR}")
        log(f"DIAGNOSTIC: Current working directory: {os.getcwd()}")
        
        # Check if we're in the right environment
        try:
            import modules
            log("DIAGNOSTIC: Successfully imported 'modules' - Oobabooga environment detected")
        except ImportError as e:
            log(f"DIAGNOSTIC: ERROR importing 'modules': {e}")
            log("DIAGNOSTIC: This suggests we're not in Oobabooga environment")
        
        # Check for required Oobabooga modules
        modules_to_check = ['shared', 'chat']
        for module_name in modules_to_check:
            try:
                exec(f"from modules import {module_name}")
                log(f"DIAGNOSTIC: Successfully imported modules.{module_name}")
            except ImportError as e:
                log(f"DIAGNOSTIC: ERROR importing modules.{module_name}: {e}")
        
        # Check if params is properly defined
        if 'params' in globals():
            log(f"DIAGNOSTIC: Extension params defined: {params}")
        else:
            log("DIAGNOSTIC: ERROR - Extension params not defined")
        
        # Check required functions
        required_functions = ['setup', 'ui', 'input_modifier', 'history_modifier']
        for func_name in required_functions:
            if func_name in globals():
                log(f"DIAGNOSTIC: Function '{func_name}' is defined")
            else:
                log(f"DIAGNOSTIC: ERROR - Function '{func_name}' is missing")
        ...
        try:
            ...
            with open(test_file, 'w') as f:
                f.write("test")
            os.remove(test_file)
            log("DIAGNOSTIC: Extension directory is writable")
        except Exception as e:
            log(f"DIAGNOSTIC: ERROR - Extension directory not writable: {e}")
        
        log("DIAGNOSTIC: Startup diagnostic complete")
        
    except Exception as e:
        print(f"[Boredom Monitor] CRITICAL ERROR in diagnostic_startup_log: {e}")
        import traceback
        print(f"[Boredom Monitor] Traceback: {traceback.format_exc()}")
...
def setup():
    ...
    log("DIAGNOSTIC: setup() function called by Oobabooga")
    log("DIAGNOSTIC: This confirms the extension is being loaded")
    ...
    log("DIAGNOSTIC: Monitor thread started")
    log("Boredom Monitor setup complete - extension ready")
    log("DIAGNOSTIC: setup() function completed successfully")

def input_modifier(string, state, is_chat=False):
    """Track user activity when input is processed"""
    global last_activity_time
    last_activity_time = time.time()
    log(f"DIAGNOSTIC: input_modifier called with: {string[:30]}...")
    return string
...
def history_modifier(history):
    ...
    global last_activity_time
    last_activity_time = time.time()
    log("DIAGNOSTIC: history_modifier called")
    return history
...
def chat_input_modifier(text, visible_text, state):
    ...
    global last_activity_time
    last_activity_time = time.time()
    log(f"DIAGNOSTIC: chat_input_modifier called with: {text[:30]}...")
...
def state_modifier(state):
    ...
    global last_activity_time
    last_activity_time = time.time()
    log("DIAGNOSTIC: state_modifier called")
    return state

def output_modifier(string, state, is_chat=False):
    """Track user activity when output is processed"""
    global last_activity_time
    last_activity_time = time.time()
    return string
...
def ui():
    ...
    log("DIAGNOSTIC: ui() function called by Oobabooga")
    log("DIAGNOSTIC: This confirms extension UI is being requested")
    ...
    log("DIAGNOSTIC: Creating Gradio UI components...")
    
    try:
        # Main container with styling similar to Coqui TTS
        with gr.Group():
            gr.Markdown("<h3 style='text-align: center;'>Boredom Monitor Extension</h3>")
            log("DIAGNOSTIC: Created main group and markdown header")
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    """)
                    log("DIAGNOSTIC: Created configuration display")
                    
                with gr.Column(scale=1):
                    # Countdown display
                    countdown_text = gr.Textbox(
                        value="7m 0s until idle trigger",
                        label="Next Idle Check",
                        interactive=False,
                        max_lines=1,
                        elem_id="boredom-countdown"
                    )
                    log("DIAGNOSTIC: Created countdown display")
            ...
            log("DIAGNOSTIC: Created console output textbox")
            ...
            with gr.Row():
                ...
                clear_console_btn = gr.Button("Clear Console", variant="secondary", size="sm")
                log("DIAGNOSTIC: Created control buttons")
                
            # Hidden refresh button for auto-updates
            refresh_btn = gr.Button("Refresh", visible=False, elem_id="boredom_refresh")
            log("DIAGNOSTIC: Created hidden refresh button")
        
        log("DIAGNOSTIC: All UI components created successfully")
        ...
        log("DIAGNOSTIC: Button event handlers set up successfully")
        log("DIAGNOSTIC: ui() function completed successfully")
        
    except Exception as e:
        log(f"DIAGNOSTIC: ERROR creating UI components: {e}")
        import traceback
        log(f"DIAGNOSTIC: UI Error traceback: {traceback.format_exc()}")
        raise

def custom_css():
    """Return minimal custom CSS for the extension"""
    log("DIAGNOSTIC: custom_css() function called")
    return """
...
def custom_js():
    ...
    log("DIAGNOSTIC: custom_js() function called")
    js_file_path = os.path.join(EXTENSION_DIR, 'script.js')
    try:
        with open(js_file_path, 'r', encoding='utf-8') as f:
            js_content = f.read()
            log("DIAGNOSTIC: Loaded custom JavaScript from script.js")
            return js_content
    except FileNotFoundError:
        log("DIAGNOSTIC: script.js not found - using minimal fallback JavaScript")
        return """
...
# Run diagnostic immediately when file is imported
diagnostic_startup_log()