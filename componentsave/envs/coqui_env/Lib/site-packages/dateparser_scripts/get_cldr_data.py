...
def _retrieve_locale_data(locale):
    ...
    field_keys_2 = ["wide", "abbreviated"]    # neglecting "narrow" to avoid problems in translation
    ...
    json_dict["february"] = list(filter(_filter_month_name,
                                        [gregorian_dict["months"][key1][key2]["2"]
                                         for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["march"] = list(filter(_filter_month_name,
                                     [gregorian_dict["months"][key1][key2]["3"]
                                      for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["april"] = list(filter(_filter_month_name,
                                     [gregorian_dict["months"][key1][key2]["4"]
                                      for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["may"] = list(filter(_filter_month_name,
                                   [gregorian_dict["months"][key1][key2]["5"]
                                    for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["june"] = list(filter(_filter_month_name,
                                    [gregorian_dict["months"][key1][key2]["6"]
                                     for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["july"] = list(filter(_filter_month_name,
                                    [gregorian_dict["months"][key1][key2]["7"]
                                     for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["august"] = list(filter(_filter_month_name,
                                      [gregorian_dict["months"][key1][key2]["8"]
                                       for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["september"] = list(filter(_filter_month_name,
                                         [gregorian_dict["months"][key1][key2]["9"]
                                          for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["october"] = list(filter(_filter_month_name,
                                       [gregorian_dict["months"][key1][key2]["10"]
                                        for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["november"] = list(filter(_filter_month_name,
                                        [gregorian_dict["months"][key1][key2]["11"]
                                         for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["december"] = list(filter(_filter_month_name,
                                        [gregorian_dict["months"][key1][key2]["12"]
                                         for key1 in field_keys_1 for key2 in field_keys_2]))

    json_dict["monday"] = [gregorian_dict["days"][key1][key2]["mon"]
                           for key1 in field_keys_1 for key2 in field_keys_2]

    json_dict["tuesday"] = [gregorian_dict["days"][key1][key2]["tue"]
                            for key1 in field_keys_1 for key2 in field_keys_2]

    json_dict["wednesday"] = [gregorian_dict["days"][key1][key2]["wed"]
                              for key1 in field_keys_1 for key2 in field_keys_2]

    json_dict["thursday"] = [gregorian_dict["days"][key1][key2]["thu"]
                             for key1 in field_keys_1 for key2 in field_keys_2]

    json_dict["friday"] = [gregorian_dict["days"][key1][key2]["fri"]
                           for key1 in field_keys_1 for key2 in field_keys_2]

    json_dict["saturday"] = [gregorian_dict["days"][key1][key2]["sat"]
                             for key1 in field_keys_1 for key2 in field_keys_2]
    ...
    json_dict["relative-type"]["1 year ago"] = [date_fields_dict[key]["relative-type--1"]
                                                for key in year_keys]

    json_dict["relative-type"]["0 year ago"] = [date_fields_dict[key]["relative-type-0"]
                                                for key in year_keys]

    json_dict["relative-type"]["in 1 year"] = [date_fields_dict[key]["relative-type-1"]
                                               for key in year_keys]

    json_dict["relative-type"]["1 month ago"] = [date_fields_dict[key]["relative-type--1"]
                                                 for key in month_keys]

    json_dict["relative-type"]["0 month ago"] = [date_fields_dict[key]["relative-type-0"]
                                                 for key in month_keys]

    json_dict["relative-type"]["in 1 month"] = [date_fields_dict[key]["relative-type-1"]
                                                for key in month_keys]

    json_dict["relative-type"]["1 week ago"] = [date_fields_dict[key]["relative-type--1"]
                                                for key in week_keys]

    json_dict["relative-type"]["0 week ago"] = [date_fields_dict[key]["relative-type-0"]
                                                for key in week_keys]

    json_dict["relative-type"]["in 1 week"] = [date_fields_dict[key]["relative-type-1"]
                                               for key in week_keys]

    json_dict["relative-type"]["1 day ago"] = [date_fields_dict[key]["relative-type--1"]
                                               for key in day_keys]

    json_dict["relative-type"]["0 day ago"] = [date_fields_dict[key]["relative-type-0"]
                                               for key in day_keys]

    json_dict["relative-type"]["in 1 day"] = [date_fields_dict[key]["relative-type-1"]
                                              for key in day_keys]

    json_dict["relative-type"]["0 hour ago"] = [date_fields_dict[key]["relative-type-0"]
                                                for key in hour_keys]

    json_dict["relative-type"]["0 minute ago"] = [date_fields_dict[key]["relative-type-0"]
                                                  for key in minute_keys]

    json_dict["relative-type"]["0 second ago"] = [date_fields_dict[key]["relative-type-0"]
                                                  for key in second_keys]

    json_dict["relative-type-regex"] = OrderedDict()

    json_dict["relative-type-regex"]["in \\1 year"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in year_keys for key2 in relative_keys])))
    ...
    json_dict["relative-type-regex"]["in \\1 month"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in month_keys for key2 in relative_keys])))
    ...
    json_dict["relative-type-regex"]["in \\1 week"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in week_keys for key2 in relative_keys])))
    ...
    json_dict["relative-type-regex"]["in \\1 day"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in day_keys for key2 in relative_keys])))
    ...
    json_dict["relative-type-regex"]["in \\1 hour"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in hour_keys for key2 in relative_keys])))
    ...
    json_dict["relative-type-regex"]["in \\1 minute"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in minute_keys for key2 in relative_keys])))
    ...
    json_dict["relative-type-regex"]["in \\1 second"] = (
        list(filter(_filter_relative_string,
                    [date_fields_dict[key1]["relativeTime-type-future"].get(key2)
                     for key1 in second_keys for key2 in relative_keys])))
