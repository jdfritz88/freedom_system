...
class DashboardGridRouterPanel:
    ...
    def build_panel(self):
        if self.floating:
            ...
            self.panel.title("Dashboard Grid Router")
    ...
    def draw_contents(self):
        for child in self.panel.winfo_children():
            child.destroy()
        label = ttk.Label(self.panel, text="Dashboard Grid Router")
        ...
        try:
            ...
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
            config['dashboard_grid_router_docked'] = not self.floating
            with open(CONFIG_PATH, 'w') as f:
                json.dump(config, f, indent=2)
        except Exception as e:
            print("[ERROR] Saving dashboard_grid_router state:", e)
    ...
    def load_state(self):
        try:
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
                self.floating = not config.get('dashboard_grid_router_docked', True)
        except Exception as e:
            print("[ERROR] Loading dashboard_grid_router state:", e)
            self.floating = False
