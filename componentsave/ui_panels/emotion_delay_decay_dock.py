...
class EmotionDelayDecayPanel:
    ...
    def build_panel(self):
        if self.floating:
            ...
            self.panel.title("Emotion Delay Decay")
    ...
    def draw_contents(self):
        for child in self.panel.winfo_children():
            child.destroy()
        label = ttk.Label(self.panel, text="Emotion Delay Decay")
        ...
        try:
            ...
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
            config['emotion_delay_decay_docked'] = not self.floating
            with open(CONFIG_PATH, 'w') as f:
                json.dump(config, f, indent=2)
        except Exception as e:
            print("[ERROR] Saving emotion_delay_decay state:", e)
    ...
    def load_state(self):
        try:
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
                self.floating = not config.get('emotion_delay_decay_docked', True)
        except Exception as e:
            print("[ERROR] Loading emotion_delay_decay state:", e)
            self.floating = False
