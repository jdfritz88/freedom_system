...
class EmotionOverlayPanel:
    ...
    def build_panel(self):
        if self.floating:
            ...
            self.panel.title("Emotion Orb Overlay")
    ...
    def draw_contents(self):
        for child in self.panel.winfo_children():
            child.destroy()
        label = ttk.Label(self.panel, text="Emotion Orb Overlay")
        ...
        try:
            ...
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
            config['emotion_overlay_docked'] = not self.floating
            with open(CONFIG_PATH, 'w') as f:
                json.dump(config, f, indent=2)
        except Exception as e:
            print(f"[ERROR] Saving overlay dock state: {e}")
    ...
    def load_state(self):
        try:
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
                self.floating = not config.get('emotion_overlay_docked', True)
        except Exception as e:
            print(f"[ERROR] Loading overlay dock state: {e}")
            self.floating = False
