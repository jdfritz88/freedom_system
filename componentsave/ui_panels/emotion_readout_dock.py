...
class EmotionReadoutPanel:
    ...
    def build_panel(self):
        if self.floating:
            ...
            self.panel.title("Emotion Readout Display")
            self.panel.geometry("250x100+100+100")
            self.panel.lift()
    ...
    def draw_contents(self):
        for child in self.panel.winfo_children():
            child.destroy()
        label = ttk.Label(self.panel, text="Emotion Readout Display")
        ...
        try:
            ...
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
            config['emotion_readout_docked'] = not self.floating
    ...
    def load_state(self):
        try:
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
                self.floating = not config.get('emotion_readout_docked', True)
        except Exception as e:
            print(f"[ERROR] Loading dock state: {e}")
            self.floating = False
