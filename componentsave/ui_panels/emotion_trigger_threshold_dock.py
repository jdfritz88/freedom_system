...
class EmotionTriggerThresholdPanel:
    ...
    def build_panel(self):
        if self.floating:
            ...
            self.panel.title("Emotion Trigger Threshold")
    ...
    def draw_contents(self):
        for child in self.panel.winfo_children():
            child.destroy()
        label = ttk.Label(self.panel, text="Emotion Trigger Threshold")
        ...
        try:
            ...
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
            config['emotion_trigger_threshold_docked'] = not self.floating
            with open(CONFIG_PATH, 'w') as f:
                json.dump(config, f, indent=2)
        except Exception as e:
            print("[ERROR] Saving emotion_trigger_threshold state:", e)
    ...
    def load_state(self):
        try:
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
                self.floating = not config.get('emotion_trigger_threshold_docked', True)
        except Exception as e:
            print("[ERROR] Loading emotion_trigger_threshold state:", e)
            self.floating = False
