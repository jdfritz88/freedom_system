...
class MasterStatusPanel:
    ...
    def build_panel(self):
        if self.floating:
            ...
            self.panel.title("Master Status")
    ...
    def draw_contents(self):
        for child in self.panel.winfo_children():
            child.destroy()
        label = ttk.Label(self.panel, text="Master Status")
        ...
        try:
            ...
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
            config['master_status_docked'] = not self.floating
            with open(CONFIG_PATH, 'w') as f:
                json.dump(config, f, indent=2)
        except Exception as e:
            print("[ERROR] Saving master_status state:", e)
    ...
    def load_state(self):
        try:
            if os.path.exists(CONFIG_PATH):
                with open(CONFIG_PATH, 'r') as f:
                    config = json.load(f)
                self.floating = not config.get('master_status_docked', True)
        except Exception as e:
            print("[ERROR] Loading master_status state:", e)
            self.floating = False
