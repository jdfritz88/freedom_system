...
# Import voice synchronization module with server conflict detection
...
if check_alltalk_server_running():
    print("[AllTalk] Standalone AllTalk server detected - using remote configuration")
    sync_all_voices = None
else:
    try:
...
if not alltalk_already_running and running_in_standalone:
    ...
    if standalone_python.exists():
        ...
        print_message(f"[AllTalk] Fallback to current Python: {sys.executable}")
        _state["process"] = subprocess.Popen([sys.executable, script_path])
        print_message(f"[AllTalk] Fallback subprocess PID: {_state['process'].pid}")

# Check if the subprocess has started successfully (or if using existing AllTalk)
...
if _state['running_on_google_colab']:
    ...
    print_message(
        "\033[94mTTS Generator:\033[00m \033[92mhttp://127.0.0.1:"
        + str(config.api_def.api_port_number)
        + "/static/tts_generator/tts_generator.html\033[00m (for large texts)"
    )
...
###################################################################
atexit.register(lambda: _state["process"].terminate() if _state["process"] is not None and _state["process"].poll() is None else None)
...
if gradio_enabled is True:
    ...
    if script_dir.name == "alltalk_tts":
        ...
        # Running as part of text-generation-webui: Add the current directory to module search path
        sys.path.insert(0, str(script_dir))
        base_package = None  # Use direct imports since current dir is in sys.path
    ...
    def load_engine_configs(_state):
        ...
        for engine_name in _state.get('srv_engines_available', []):
            # Always use simple relative imports since current directory is in sys.path
            module_name = f"system.tts_engines.{engine_name}.{engine_name}_settings_page"
            module = dynamic_import(module_name, package=None)
            if module:
    ...
    def generate_tts(
        ...
        while retries < max_retries:
            try:
                print_message(f"[AllTalk] Sending form data payload: {tts_data}", message_type="debug_tts_variables")
                tts_response = requests.post(api_url, data=tts_data, timeout=60)
                print_message(f"[AllTalk] Server response status: {tts_response.status_code}", message_type="debug_tts_variables")
                print_message(f"[AllTalk] Server response body: {tts_response.text}", message_type="debug_tts_variables")
                ...
                if retries == max_retries:
                    if isinstance(e, requests.exceptions.Timeout):
                        ...
                        print_message(f"[AllTalk] HTTP Error Details: {str(e)}", message_type="error")
                        if hasattr(e.response, 'text'):
                            print_message(f"[AllTalk] Server error message: {e.response.text}", message_type="error")
        ...
        with gr.Blocks(
                ...
                if _state['running_in_docker']:
                    with gr.Row():
                        with gr.Accordion(
                            ...
                            with gr.Accordion(
                                "HELP - Docker IP/URL for API Address", open=False
                            ):
                                with gr.Row():
                                    gr.Markdown(
            ...
            with gr.Tab("Global Settings"):
                ...
                with gr.Tab("AllTalk API Defaults"):
                    ...
                    with gr.Accordion(
                        "HELP - - Request Configuration (Character limits, language, file names)",
                        open=False,
                    ):
                        gr.Markdown(
                    ...
                    with gr.Accordion(
                        "HELP - - Narrator & Playback (Narrator modes, text handling, playback options",
                        open=False,
                    ):
                        gr.Markdown(
