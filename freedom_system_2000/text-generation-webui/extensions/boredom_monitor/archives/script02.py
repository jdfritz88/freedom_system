...
# Debug version to diagnose shared.state issue
# ==========================================
...
params = {
    ...
    "is_tab": False,  # Changed to False - will appear below chat
}
...
def inject_idle_message():
    """Inject an idle message into the chat - DEBUG VERSION"""
    try:
        ...
        # DEBUG: Check what's actually in shared module
        log("=== DEBUG: Checking shared module ===")
        log(f"shared module attributes: {dir(shared)}")
        
        if hasattr(shared, 'state'):
            log(f"shared.state exists: {type(shared.state)}")
            log(f"shared.state contents: {shared.state}")
        else:
            log("shared.state does NOT exist")
            
        # Look for other state-like attributes
        state_attrs = [attr for attr in dir(shared) if 'state' in attr.lower()]
        log(f"State-related attributes: {state_attrs}")
        
        # Check for history-related attributes
        history_attrs = [attr for attr in dir(shared) if 'history' in attr.lower()]
        log(f"History-related attributes: {history_attrs}")
        
        # Check for args or settings
        if hasattr(shared, 'args'):
            log(f"shared.args exists: {type(shared.args)}")
        if hasattr(shared, 'settings'):
            log(f"shared.settings exists: {type(shared.settings)}")
        if hasattr(shared, 'model'):
            log(f"shared.model exists: {type(shared.model)}")
        if hasattr(shared, 'tokenizer'):
            log(f"shared.tokenizer exists: {type(shared.tokenizer)}")
            
        log("=== END DEBUG ===")
        
        # Simple fallback - just log the injection
        log(f"Template would be: {template}")
        log("Injection simulated (no actual chat modification)")
...
def ui():
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            gr.Markdown(f"**Settings:** Idle trigger: {idle_minutes} minutes | Status: {'Active' if should_monitor.is_set() else 'Inactive'}")
    ...
    with gr.Accordion("Recent Activity Log", open=False):
        try:
            with open(TEMPLATE_LOG, 'r', encoding='utf-8') as f:
                log_content = f.read()
            if log_content.strip():
                gr.Code(log_content, label="Template Usage Log", language="text", max_lines=10)
            else:
